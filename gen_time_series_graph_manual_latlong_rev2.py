...
import os, glob, sys, rasterio, pyproj, csv
...
smpls = [
        (66.020665, -154.133065), 
        (66.187050, -153.932029),
        (65.979228, -154.049494),
        (65.920039, -154.040912), 
]

def convertLL(lat, long):
    # Convert the lat/long point of interest to a row/col location
    template_tif_list = glob.glob(os.path.join(in_dir, 'wsa', 'MCD43A3.A*{tile}*wsa_shortwave.tif'.format(tile=tile)))
    template_tif = template_tif_list[0]
    template_raster = rasterio.open(template_tif)
    in_proj = pyproj.Proj(init='epsg:4326')
    out_proj = pyproj.Proj(template_raster.crs)
    x, y = pyproj.transform(in_proj, out_proj, long, lat)
    ...
    for year in years:
        ...
        for day in doys:
            ...
            if len(wsa_tif_list) == 0 or len(bsa_tif_list) == 0 or len(qa_tif_list) == 0:
                ...
                smpl_results = []
                for smpl in smpls:
                    smp_rc = convertLL(smpl[0], smpl[1])
                    print(smp_rc)
